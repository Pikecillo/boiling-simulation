...
void HeatField::latent_heat_update(const Matrixd &field0, Matrixd &field) {
  for(size_t i = 1; i < field0.rows(); i++)
    for(size_t j = 1; j + 1 < field0.cols(); j++){
      ...
      field.at(i, j) += ((to_steam ? latent_heat : 0) -
  ...
  for(size_t i = 0; i < field0.rows(); i++)
    for(int j = 0; j < field0.cols(); j++) {
      ...
      if(phase_function(field0.at(i, j)) *
         phase_function(field0.at(i, j - 1)) < 0.0 ||
         phase_function(field0.at(i, j)) *
         phase_function(field0.at(i, j + 1)) < 0.0 ||
         phase_function(field0.at(i, j)) *
         phase_function(field0.at(i - 1, j)) < 0.0 ||
         phase_function(field0.at(i, j)) *
         phase_function(field0.at(i + 1, j)) < 0.0) {
...
double HeatField::phase_function(double T){
...
void HeatField::add_vectors(double a, double v1[],
		 double b, double v2[],
		 double v[]){
  v[0] = a * v1[0] + b * v2[0];
  v[1] = a * v1[1] + b * v2[1];
...
double HeatField::module(double v[]){

  return sqrt(v[0] * v[0] + v[1] * v[1]);
}

double HeatField::curvature(const Matrixd &heat,
		 double px, double py,
		 double nx, double ny){
