    ...
    for (size_t i = 1; i + 1 < field0.rows(); i++)
        for (size_t j = 1; j + 1 < field0.cols(); j++)
        {
            ...
            if (i == 0 || i == field0.rows() - 1)
                ...
                field.at(i, j) = field0.at(i, j) +
                                 quarter_diffusion * (field0.at(i, j - 1) + field0.at(i, j + 1) + field0.at(i - 1, j) +
                                                      field0.at(i + 1, j) - 4.0 * field0.at(i, j));
        }
}
...
void HeatField::buoyancy_update(const Matrixd &field0, Matrixd &field)
{
    ...
    for (size_t i = 1; i + 1 < field0.rows(); i++)
        for (size_t j = 1; j + 1 < field0.cols(); j++)
        {
            field.at(i, j) =
                field0.at(i, j) *
                (1 - half_buoyancy * (phase_function(field0.at(i + 1, j)) - phase_function(field0.at(i - 1, j))));
        }
}
...
void HeatField::latent_heat_update(const Matrixd &field0, Matrixd &field)
{
    for (size_t i = 1; i < field0.rows(); i++)
        for (size_t j = 1; j + 1 < field0.cols(); j++)
        {
            ...
            field.at(i, j) += ((to_steam ? latent_heat : 0) - (to_water ? latent_heat : 0));
        }
}
...
void HeatField::add_vectors(float a, float v1[], float b, float v2[], float v[])
{
    v[0] = a * v1[0] + b * v2[0];
    v[1] = a * v1[1] + b * v2[1];
}
...
float HeatField::module(float v[])
{
    return sqrt(v[0] * v[0] + v[1] * v[1]);
}

float HeatField::curvature(const Matrixd &heat, float px, float py, float nx, float ny)
{
    float kpn, np[2], nn[2], p[] = {px, py}, n[] = {nx, ny}, t1[2], t2[2], t3[2], t4[2];
